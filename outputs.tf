...
output "container_registry_credential_sets_authentication_credentials" {
  ...
  value       = { for k, v in azurerm_container_registry_credential_set.container_registry_credential_sets : k => one(v.authentication_credentials) if v.authentication_credentials != null && length(v.authentication_credentials) > 0 }
}
...
output "container_registry_credential_sets_identity" {
  ...
  value       = { for k, v in azurerm_container_registry_credential_set.container_registry_credential_sets : k => one(v.identity) if v.identity != null && length(v.identity) > 0 }
}
